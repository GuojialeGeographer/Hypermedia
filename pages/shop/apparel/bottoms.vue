  ...
  <div class="min-h-screen bg-white">
    ...
    <div class="max-w-[1920px] mx-auto px-4 pt-20">
      ...
      <div class="flex items-center space-x-2 text-[25px] pt-6 pb-8">
        ...
        <span class="text-black">/</span>
        <span class="text-black font-medium">Bottoms</span>
      </div>

      <!-- Page Title -->
      <h1 class="text-[66px] font-bold text-black text-center mb-16">Bottoms</h1>
      ...
      <div class="flex">
        ...
        <div class="flex-1">
          ...
          <div class="flex items-center justify-between mb-8">
            ...
            <span class="text-[36px] font-bold text-[#666666]">{{ bottomProducts.length }} products</span>
          </div>

          <!-- Product Grid -->
          <div class="grid grid-cols-3 gap-x-8 gap-y-16">
            <div v-for="product in bottomProducts" :key="product.id" class="group">
              <!-- Product Image -->
...
const bottomProducts = computed(() => {
  if (!products.value) return [];
  return products.value.filter(product => product.category === 'bottom');
});

useHead({
  title: 'Bottoms - Yoga Studio Shop',
  meta: [
    { name: 'description', content: 'Shop our collection of yoga bottoms.' }
  ]
});
</script> 