  ...
  <div class="bg-white font-inter">
    ...
    <main v-else class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <nav class="flex items-center space-x-2 text-gray-500 mb-8">
        ...
        <span>/</span>
        <span class="text-gray-700 font-medium">{{ course?.title || 'Ashtanga Fundamentals' }}</span>
      </nav>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div class="space-y-6 text-lg">
            ...
            <div>
              ...
              <p class="text-gray-600 leading-relaxed">This instructor is currently teaching <span class="font-bold text-gray-800">{{ course?.title || 'Ashtanga Fundamentals' }}</span>.</p>
...
const currentCourseSlug = 'ashtanga-fundamentals';
const currentCourseName = 'Ashtanga Fundamentals';
...
const mainTeacher = computed(() => {
  ...
  // Based on the data, Liam O'Connell teaches Ashtanga Fundamentals.
  return allTeachers.value.find(t => t.name === 'Liam O\'Connell') || null;
});
...
watchEffect(() => {
  ...
  const description = mainTeacher.value 
    ? `Learn about the Ashtanga Fundamentals course taught by ${mainTeacher.value.name}.` 
    : 'Details about the Ashtanga Fundamentals course.';
