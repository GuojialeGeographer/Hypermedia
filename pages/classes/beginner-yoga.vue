  ...
  <div class="bg-white font-inter">
    ...
    <main v-else class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <nav class="flex items-center space-x-2 text-gray-500 mb-8">
        ...
        <span>/</span>
        <span class="text-gray-700 font-medium">{{ course?.title || 'Beginner Yoga' }}</span>
      </nav>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div class="space-y-6 text-lg">
            ...
            <div>
              ...
              <p class="text-gray-600 leading-relaxed">This instructor is currently teaching <span class="font-bold text-gray-800">{{ course?.title || 'Beginner Yoga' }}</span>.</p>
...
// Temporarily define Course type here until it's globally available
interface Course {
  slug: string;
  title: string;
  intensity: number;
...
const currentCourseSlug = 'beginner-yoga';
const currentCourseName = 'Beginner Yoga Course Introduction';
...
const mainTeacher = computed(() => {
  ...
  return allTeachers.value.find(t => t.courses?.includes(currentCourseName)) || null;
});

const otherTeachers = computed(() => {
  if (!allTeachers.value || !mainTeacher.value) return [];
  // Ensure we don't include the main teacher and limit the results
  return allTeachers.value.filter(t => t.id !== mainTeacher.value?.id).slice(0, 5);
});

const pending = computed(() => {
  // Check if either fetch is still pending
...
watchEffect(() => {
  ...
  const description = mainTeacher.value 
    ? `Learn about the Beginner Yoga course taught by ${mainTeacher.value.name}.` 
    : 'Details about the Beginner Yoga course.';
