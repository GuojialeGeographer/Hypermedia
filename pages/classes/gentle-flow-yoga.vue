  ...
  <div class="bg-white font-inter">
    ...
    <main v-else class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <nav class="flex items-center space-x-2 text-gray-500 mb-8">
        ...
        <span>/</span>
        <span class="text-gray-700 font-medium">{{ course?.title || 'Gentle Flow Yoga' }}</span>
      </nav>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div class="space-y-6 text-lg">
            ...
            <div>
              ...
              <p class="text-gray-600 leading-relaxed">This instructor is currently teaching <span class="font-bold text-gray-800">{{ course?.title || 'Gentle Flow Yoga' }}</span>.</p>
...
const currentCourseSlug = 'gentle-flow-yoga';
const currentCourseName = 'Gentle Flow Yoga';
...
const mainTeacher = computed(() => {
  ...
  // Based on the data, Georgia Weibel teaches Gentle Flow Yoga.
  return allTeachers.value.find(t => t.name === 'Georgia Weibel') || null;
});
...
watchEffect(() => {
  const title = mainTeacher.value 
    ...
    : 'Gentle Flow Yoga Course';
  
  const description = mainTeacher.value 
    ? `Learn about the Gentle Flow Yoga course taught by ${mainTeacher.value.name}.` 
    : 'Details about the Gentle Flow Yoga course.';
